...
GO_BASE_TAGS ?= containers_image_openpgp netgo static osusergo
GO_DRIVER_TAGS ?= exclude_graphdriver_devicemapper exclude_graphdriver_btrfs
ifeq ($(ENABLE_BTRFS),1)
	GO_DRIVER_TAGS := $(filter-out exclude_graphdriver_btrfs,$(GO_DRIVER_TAGS))
endif
...
GO_BUILD_FLAGS += -tags "$(GO_TAGS)" -trimpath -ldflags "$(GO_LDFLAGS)"
...
.PHONY: go.coverage
go.coverage:
	@$(GO) test -tags "$(GO_TAGS)" -race -failfast -coverprofile=coverage.out -covermode=atomic `go list ./pkg/env ./pkg/apply  | grep -v "/test\|/fork"`
