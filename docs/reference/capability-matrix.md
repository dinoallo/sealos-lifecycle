...
| Live `ReleaseChannel` release lookup and health-gated promotion service | Not implemented | Local file-backed `ReleaseChannel` resolution and `sealos sync promote` advancement exist, but there is no registry/API-backed lookup by `distribution line + channel` and no health-gated promotion service. |
