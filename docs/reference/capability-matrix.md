...
| Push a built OCI component package image and capture its digest | Ready now | `sealos sync package push` in [cmd/sealos/cmd/sync_package.go](../../cmd/sealos/cmd/sync_package.go), tests in [cmd/sealos/cmd/sync_package_test.go](../../cmd/sealos/cmd/sync_package_test.go) | Push validates the returned OCI digest and can write provenance, but it does not update any BOM automatically or generate signatures. |
...
| Inspect and garbage-collect the package cache | Ready now | `sealos sync package cache list/gc` in [cmd/sealos/cmd/sync_package.go](../../cmd/sealos/cmd/sync_package.go), cache retention logic in [pkg/distribution/ocipackage/pull.go](../../pkg/distribution/ocipackage/pull.go) | GC removes invalid entries by TTL and keeps valid package entries unless `--include-valid` is set, preserving last known good package sets for registry outage, offline mirror, and rollback workflows. |
