...
| Promote a local `DistributionChannel` to a target BOM revision | Ready now | `sealos sync promote` in [cmd/sealos/cmd/sync.go](../cmd/sealos/cmd/sync.go), promotion helper in [pkg/distribution/bom/channel.go](../pkg/distribution/bom/channel.go) | This updates one local channel file, validates the target BOM belongs to the same line, and appends `spec.promotionHistory[]`. It is not registry/API-backed release lookup or health-gated promotion automation. |
