...
| Promote a local `DistributionChannel` to a target BOM revision with optional health proof | Ready now | `sealos sync promote` in [cmd/sealos/cmd/sync.go](../cmd/sealos/cmd/sync.go), `DistributionHealthProof` and promotion helper in [pkg/distribution/bom/channel.go](../pkg/distribution/bom/channel.go) | This updates one local channel file, validates the target BOM belongs to the same line, optionally requires a passed local `DistributionHealthProof` for that exact line/revision, and appends `spec.promotionHistory[]` with proof metadata. It is not registry/API-backed release lookup or a health-proof ingestion service. |
...
| Batch host-targeted rendered-bundle rollout with optional health gate | Ready with boundary | `--rollout-batch-size` and `--rollout-health-gate` in [cmd/sealos/cmd/sync.go](../cmd/sealos/cmd/sync.go) and [cmd/sealos-agent/cmd/root.go](../cmd/sealos-agent/cmd/root.go), `DistributionRolloutPolicy` in [pkg/distribution/controller/types.go](../pkg/distribution/controller/types.go), batching in [pkg/distribution/reconcile/apply.go](../pkg/distribution/reconcile/apply.go) | Batching applies to eligible host-targeted all-node runtime-rootfs steps inside the rendered-bundle executor. When `healthGate` is enabled, component `healthcheck` hooks run after each host batch before the next batch advances. Controller targets can persist batch size and health gate through a same-namespace `DistributionRolloutPolicy`; this is not yet canary pauses or automatic rollback. |
...
| Canary or rollback-capable rollout policy | Not implemented | Durable batch-size and per-batch health gate policy now exists for eligible controller targets, but there is still no package-level safety model, canary pause, or automatic rollback model for every multi-node workflow. |
...
- local `DistributionChannel` advancement with explicit promotion history
