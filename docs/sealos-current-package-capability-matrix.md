...
| Batch host-targeted rendered-bundle rollout | Ready with boundary | `--rollout-batch-size` in [cmd/sealos/cmd/sync.go](../cmd/sealos/cmd/sync.go) and [cmd/sealos-agent/cmd/root.go](../cmd/sealos-agent/cmd/root.go), `DistributionRolloutPolicy` in [pkg/distribution/controller/types.go](../pkg/distribution/controller/types.go), batching in [pkg/distribution/reconcile/apply.go](../pkg/distribution/reconcile/apply.go) | Batching applies to host-targeted all-node steps inside the rendered-bundle executor. Controller targets can persist the batch size through a same-namespace `DistributionRolloutPolicy`; this is not yet canary pauses, health gates, or automatic rollback. |
...
| Health-gated or rollback-capable rollout policy | Not implemented | Durable batch-size policy now exists for controller targets, but there is still no package-level safety model, canary pause, health gate, or automatic rollback model for every multi-node workflow. |
...
- health-gated rollout and release-system automation are not ready yet
