...
| [sealos-distribution-controller-install.md](./sealos-distribution-controller-install.md) | Shows how to install and upgrade the current `DistributionTarget` CRD, RBAC, and `sealos-agent --controller` deployment, then create a minimal target. | Install guide with [Chinese translation](./sealos-distribution-controller-install.zh-CN.md). |
...
3. Read [sealos-distribution-controller-install.md](./sealos-distribution-controller-install.md) when you want to install or upgrade the current minimal `DistributionTarget` controller path.
