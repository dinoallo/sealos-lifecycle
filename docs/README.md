...
5. Read [sealos-bom-and-distribution-channel-guide.md](./sealos-bom-and-distribution-channel-guide.md) when you want the object model for BOM revisions, distribution lines, and `DistributionChannel`.
6. Read [sealos-local-repo-and-secret-guide.md](./sealos-local-repo-and-secret-guide.md) when you want a concrete local-repo layout and secret-initialization workflow.
7. Use [sealos-multi-cluster-distribution-and-config-sync-implementation-plan.md](./sealos-multi-cluster-distribution-and-config-sync-implementation-plan.md) for repo implementation sequencing.
8. Read [sealos-cilium-packaging-walkthrough.md](./sealos-cilium-packaging-walkthrough.md) when you want a concrete example of the current packaging flow.
9. Read [sealos-grafana-kubeblocks-example.md](./sealos-grafana-kubeblocks-example.md) when you want a stateful application example that includes a database boundary and Secret handling.
10. Read [sealos-derived-distribution-walkthrough.md](./sealos-derived-distribution-walkthrough.md) when you want the supported conceptual path for a cluster to fork into a derived distribution line.
11. Use [sealos-oci-component-packaging-milestone-plan.md](./sealos-oci-component-packaging-milestone-plan.md) to understand the OCI packaging milestone that has already been proven in this repo.
12. Use [sealos-minimal-k8s-package-poc-plan.md](./sealos-minimal-k8s-package-poc-plan.md) when you need the prepared-host PoC flow and validation shape.
