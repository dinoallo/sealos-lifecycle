...
| 用可选 health proof 把本地 `DistributionChannel` 推进到目标 BOM revision | Ready now | [sync.go](../cmd/sealos/cmd/sync.go) 里的 `sealos sync promote`，以及 [channel.go](../pkg/distribution/bom/channel.go) 里的 `DistributionHealthProof` 和 promotion helper | 它会更新一份本地 channel 文件，校验目标 BOM 属于同一条线，可选要求一份针对同一 line/revision 且通过的本地 `DistributionHealthProof`，并把 proof 元数据追加进 `spec.promotionHistory[]`。这不是 registry/API-backed release lookup，也不是 health-proof ingestion service。 |
...
- 带显式 promotion history 的本地 `DistributionChannel` advancement
- 进程级 agent reconcile
