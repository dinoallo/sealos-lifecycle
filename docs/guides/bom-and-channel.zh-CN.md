...
| 主题 | 当前仓库行为 | 目标设计方向 |
| --- | --- | --- |
| 集群怎么选择目标 | 显式 BOM 文件，或通过 `--release-channel` 传入本地 `ReleaseChannel` 文件 | 显式选 BOM revision，或做 `distribution line + ReleaseChannel` lookup |
| channel 元数据放哪里 | `BOM.spec.channel`；使用 `ReleaseChannel` 文件时，render provenance 还会记录本地 channel 选择元数据 | 独立的 `ReleaseChannel` 对象 |
| `sync render` 今天解析什么 | 直接传入的 BOM 文档，或本地 `ReleaseChannel`，它的 `spec.bomPath` 指向要加载的 BOM | 先做可选的 channel lookup，再落到具体 BOM revision |
| applied state 今天记录什么 | BOM name、revision、channel；rendered bundle 还会记录 BOM 和本地 `ReleaseChannel` provenance | BOM name、revision，以及把它解析出来的 channel 或显式 target |
...
`sealos sync promote`。它会把一份本地 `ReleaseChannel` 文件推进到目标 BOM
...
- registry/API 驱动的 `ReleaseChannel` lookup
