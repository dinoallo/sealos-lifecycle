...
本文提出一种 Git 仓库组织方式，用来同步 Sealos 发行版配置，包括 package manifest、构建 class、发行版 profile、BOM 文件和发行通道。
...
- 区分 package 定义、build class、distribution profile、release BOM 和 channel 指针。
...
该仓库应保存构建、校验、选择和渲染发行版所需的源文件。不应保存生成的 render 结果、下载后的 OCI artifact 内容或本地构建出的 package artifact。
...
| `classes/<class>/<version>.yaml` | package 使用的可复用 build 或 render class 定义。 |
...
5. 校验被引用的 build class 和 package source path。
...
- 每个 build class 引用都指向已批准的 class definition
