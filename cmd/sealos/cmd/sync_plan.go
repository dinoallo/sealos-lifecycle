...
func newSyncPlanCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			preflight, err := syncApplyPreflight(
				flags.clusterName,
				bundlePath,
				flags.allowStaleTopology,
				flags.allowStaleRenderInputs,
			)
			if err != nil {
				return err
			}
	...
	cmd.Flags().
		StringVarP(&flags.clusterName, "cluster", "c", "default", "name of cluster to plan desired state for")
	cmd.Flags().
		StringVar(&flags.bundleDir, "bundle-dir", "", "path to a rendered bundle directory; defaults to the cluster current bundle")
	cmd.Flags().
		BoolVar(&flags.allowStaleTopology, "allow-stale-topology", false, "show the plan with stale executionTopology treated as an allowed preflight condition")
	cmd.Flags().
		BoolVar(&flags.allowStaleRenderInputs, "allow-stale-render-inputs", false, "show the plan with stale render inputs treated as an allowed preflight condition")
	addSyncOutputFlag(cmd, &flags.output)
	return cmd
}

func newSyncPlanOutput(
	...
	out.LocalResources = buildSyncPlanLocalResources(
		bundle.Spec.LocalResources,
		out.KubernetesObjects,
	)
...
func syncPlanTargetForStep(
	topology *syncExecutionTopology,
	step hydrate.RenderedStep,
) syncPlanResolvedTarget {
	switch step.Kind {
	case hydrate.StepHook:
	...
	case hydrate.StepContent:
		...
		default:
			return syncPlanResolvedTarget{
				Effective: string(step.ContentType),
				Scope:     "unsupported",
				Error:     fmt.Sprintf("unsupported content type %q", step.ContentType),
			}
		}
	default:
		return syncPlanResolvedTarget{
			Effective: string(step.Kind),
			Scope:     "unsupported",
			Error:     fmt.Sprintf("unsupported step kind %q", step.Kind),
		}
	}
}

func resolveSyncPlanTarget(
	topology *syncExecutionTopology,
	declared, effective packageformat.ExecutionTarget,
) syncPlanResolvedTarget {
...
func syncPlanPackageSafetyForComponent(component hydrate.RenderedComponent) syncPlanPackageSafety {
	...
	if len(component.LocalPatches) > 0 {
		...
		addUniqueString(
			&safety.Reasons,
			seenReasons,
			"local patches mutate package output and require policy approval",
		)
	}
	...
	if safety.ManifestOnly && !safety.HasRootfs && !safety.HasHostFile &&
		len(safety.HostWaveSteps) == 0 {
		...
		addUniqueString(
			&safety.Reasons,
			seenReasons,
			"package only changes Kubernetes API objects and must pass cluster-level health gates",
		)
		return safety
	}
	if safety.HasRootfs {
		...
		addUniqueString(
			&safety.Reasons,
			seenReasons,
			"rootfs content mutates host filesystem and is protected by host batches",
		)
		return safety
	}
	if safety.HasHostFile {
		...
		addUniqueString(
			&safety.Reasons,
			seenReasons,
			"host file content mutates host filesystem and is protected by host batches",
		)
		return safety
	}
	if safety.HasPackageHook {
		...
		addUniqueString(
			&safety.Reasons,
			seenReasons,
			"package hooks execute package-specific logic and must declare explicit targets",
		)
		return safety
	}
	return safety
...
func syncPlanStepSafetyForStep(
	step hydrate.RenderedStep,
	target syncPlanResolvedTarget,
) syncPlanStepSafety {
	...
	case step.Kind == hydrate.StepContent && step.ContentType == packageformat.ContentValues:
		...
		safety.Reasons = []string{
			"values content is consumed during render and is not applied directly",
		}
	case step.Kind == hydrate.StepContent && step.ContentType == packageformat.ContentPatch:
		...
		safety.Reasons = []string{
			"patch content changes rendered package output and requires policy approval before apply",
		}
		safety.RequiresApproval = true
	case step.Kind == hydrate.StepContent && step.ContentType == packageformat.ContentChart:
		...
		safety.Reasons = []string{
			"chart content renders Kubernetes API objects and must pass cluster-level health gates",
		}
	case step.Kind == hydrate.StepHook:
...
func syncPlanHookSafety(
	...
	case packageformat.PhaseBootstrap:
		...
		safety.Reasons = append(
			safety.Reasons,
			"bootstrap hooks run before package install completion",
		)
	case packageformat.PhaseConfigure:
		...
		safety.Reasons = append(
			safety.Reasons,
			"configure hooks can mutate runtime configuration and must be followed by health gates",
		)
	case packageformat.PhaseInstall:
		...
		safety.Reasons = append(
			safety.Reasons,
			"install hooks can create runtime resources and must be followed by health gates",
		)
	case packageformat.PhaseUpgrade:
		...
		safety.Reasons = append(
			safety.Reasons,
			"upgrade hooks can migrate live state and require rollback planning",
		)
		safety.RequiresApproval = true
	case packageformat.PhaseRemove:
		...
		safety.Reasons = append(
			safety.Reasons,
			"remove hooks can delete runtime resources and require explicit approval",
		)
		safety.RequiresApproval = true
...
func buildSyncPlanK8sObjects(
	objects []hydrate.TrackedK8sObject,
	summary *syncPlanSummary,
) []syncPlanK8sObject {
...
func buildSyncPlanLocalResources(
	paths []string,
	objects []syncPlanK8sObject,
) []syncPlanLocalResource {
	if len(paths) == 0 {
		return nil
	}
...
func buildSyncPlanHostPathSets(
	paths []hydrate.TrackedHostPath,
	topology *syncExecutionTopology,
) []syncPlanHostPathSet {
	if len(paths) == 0 {
		return nil
...
func syncPlanHostsForTrackedHostPath(
	topology *syncExecutionTopology,
	path hydrate.TrackedHostPath,
) []string {
	if topology == nil {
